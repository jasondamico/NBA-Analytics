...
class BBallReferenceScraper:
    ...
    def convert_bdl_season_to_bball_ref(self, season):
        """
    ...
    def check_status_code(self, response):
        ...
        if response.status_code in range (400, 499):
            if response.status_code == 404:
                raise Exception("404 Error: Page not found. Please check to see if there is data available for the %d season." % (self.get_season))
            else:
                exception_message = "%d Error: %s" % (response.status_code, response.reason)
                raise Exception(exception_message)

    def get_rows_dict(self, trs, store_new_player_func=None, update_player_func=None):
        """
